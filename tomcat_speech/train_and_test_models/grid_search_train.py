                 ...
                 for outd in params.output_dim:
                    ...
                    with open(os.path.join(output_path, "log"), "a") as f:
                        if not config.DEBUG:
                            ...
                            #this_model_params.spec_out_dim = spec_out
                            ...
                            this_best_f1s.append(f"LR{l_rate}_DPT{dpt}_FHID{fhid}_BATCH{b_size}_specout{spec_out}_outdim{outd}")
                            all_best_f1s.append(this_best_f1s)
                            # wf.write(f"{','.join(this_best_f1s)\n}"
                            print(f"BEST F1S ARE {','.join([str(item) for item in best_f1s])}")
