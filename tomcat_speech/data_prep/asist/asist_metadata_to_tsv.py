...
def convert_metadata(metadata_file_path, save_path, n_speakers=1):
    ...
    with open(save_path, "w") as save_file:
        save_file.write(
            "id\tparticipant\tstart_time\tend_time\ttext" + "\temotion\ttrait\n"
        )
        for line in all_lines:
            if line["data"]["is_final"] == True:
                ...
                save_file.write(
                    f"{id}\t{participant}\t{start_time}\t{end_time}"
...
def save_data_to_file(json_data_lines, save_path, header, speaker_name):
    ...
    with open(save_path, "w") as save_file:
        ...
        for line in json_data_lines:
            if (
                line["msg"]["source"] == "tomcat_speech_analyzer"
                and line["topic"] == "agent/asr/final"
                and speaker_name in line["data"]["participant_id"]
            ):
                id = line["data"]["id"]
                text = line["data"]["text"]
